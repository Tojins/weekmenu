...
test.describe('Menu Selector', () => {
  ...
  test('can access menu selector when authenticated', async ({ page }) => {
    ...
    // Should see menu selector elements - look for recipe cards or the page title
    await expect(page.locator('.recipe-card').first().or(page.getByText('Week Menu'))).toBeVisible({ timeout: 10000 });
  });
  ...
  test('menu data persists in localStorage', async ({ page }) => {
    ...
    // Wait for recipes to load automatically
